...
internal sealed class NorInstruction(RegisterID rd, RegisterID rs, RegisterID rt, int lineIndex): RTypeInstruction(rd, rs, rt, lineIndex) {
    public override void Execute(ExecuteContext context) {
        ...
        context.Log($"nor ${this.Rd}, ${this.Rs}, ${this.Rt}: ~(0x{rsVal:X8} | 0x{rtVal:X8}) = 0x{result:X8}");
    }
}

internal sealed class NorInstructionParser: IInstructionParser {
    public string Mnemonic => "nor";
    ...
    public bool TryParse(string operands, int lineIndex, [MaybeNullWhen(false)] out IInstruction instruction) {
        ...
        if(RTypeInstruction.TryParse3RegOperands(operands, out RegisterID rd, out RegisterID rs, out RegisterID rt)) {
            instruction = new NorInstruction(rd, rs, rt, lineIndex);
            return true;
        }
        return false;
    }
