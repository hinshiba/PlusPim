...
internal sealed class OrInstruction(RegisterID rd, RegisterID rs, RegisterID rt, int lineIndex): RTypeInstruction(rd, rs, rt, lineIndex) {
...
internal sealed class OrInstructionParser: IInstructionParser {
    ...
    public bool TryParse(string operands, int lineIndex, [MaybeNullWhen(false)] out IInstruction instruction) {
        ...
        if(RTypeInstruction.TryParse3RegOperands(operands, out RegisterID rd, out RegisterID rs, out RegisterID rt)) {
            instruction = new OrInstruction(rd, rs, rt, lineIndex);
            return true;
        }
        return false;
    }
