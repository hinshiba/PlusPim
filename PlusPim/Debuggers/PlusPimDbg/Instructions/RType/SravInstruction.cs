...
internal sealed class SravInstruction(RegisterID rd, RegisterID rs, RegisterID rt, int lineIndex): RTypeInstruction(rd, rs, rt, lineIndex) {
    public override void Execute(ExecuteContext context) {
        ...
        context.Log($"srav ${this.Rd}, ${this.Rt}, ${this.Rs}: 0x{rtVal:X8} >> {rsVal & 0x1F} = 0x{result:X8}");
    }
}

internal sealed class SravInstructionParser: IInstructionParser {
    public string Mnemonic => "srav";
    ...
    public bool TryParse(string operands, int lineIndex, [MaybeNullWhen(false)] out IInstruction instruction) {
        ...
        if(RTypeInstruction.TryParse3RegOperands(operands, out RegisterID rd, out RegisterID parsedRs, out RegisterID parsedRt)) {
            instruction = new SravInstruction(rd, parsedRt, parsedRs, lineIndex);
            return true;
        }
        return false;
    }
